...
class VisualizationPlot(object):
    def __init__(self, arguments, read_tracks, static_info, meta_dictionary, current_frame=1, relevant_object=0, surrounding_objects=[], fig=None):
        ...
        self.surrounding_objects = surrounding_objects
        self.decision = "None"
        ...
        self.ax_button_take_sample = self.fig.add_axes([0.6, 0.035, 0.025, 0.07])
        self.ax_button_reject_sample = self.fig.add_axes([0.65, 0.035, 0.025, 0.07])
        self.ax_button_close = self.fig.add_axes([0.8, 0.035, 0.025, 0.07])
        ...
        self.button_take_sample = Button(self.ax_button_take_sample, 'Take Sample')
        self.button_reject_sample = Button(self.ax_button_reject_sample, 'Reject Sample')
        self.button_close = Button(self.ax_button_close, 'CLOSE')
        ...
        self.button_take_sample.on_clicked(self.take_sample)
        self.button_reject_sample.on_clicked(self.reject_sample)
        self.button_close.on_clicked(self.close_command)
    ...
    def reject_sample(self, _):
        self.decision = "Reject"
        plt.close()

    def close_command(self, _):
        self.decision = "Close"
        plt.close()
    ...
    def update_figure(self):
        ...
        for track in self.tracks:
            ...
            if track_id == self.relevant_object:
                rect_style = dict(facecolor="r", fill=True, edgecolor="k", zorder=19)
                track_style = dict(color="r", linewidth=1, zorder=10)
            elif track_id in self.surrounding_objects:
                rect_style = dict(facecolor="b", fill=True, edgecolor="k", zorder=19)
                track_style = dict(color="b", linewidth=1, zorder=10)
            else:
                rect_style = dict(facecolor="grey", fill=True, edgecolor="k", zorder=19)
                track_style = dict(color="grey", linewidth=1, zorder=10)
